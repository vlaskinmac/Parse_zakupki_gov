...
def get_content_by_segments(period_segment, start_date_str, page, per_page):
    ...
    for segment in count(step=14):
        ...
        # print(response.url)
        logger.warning(f"\n ======= {start_segment_date} --- {end_segment_date} =======")
        ...
        if end_segment_date_obj > datetime.datetime.strptime('05.01.2019', '%d.%m.%Y'):
...
async def create_tasks_soap(period_segment, start_date_str, page, per_page):
    ...
    for i in contaner_links:
        ...
        print(rand_conn, ' --conn |', rand_s, '--semaphore | ', count_links_total, '--links_total |', count_line_total, '--count_line_total')
...
def main():
    ...
    asyncio.run(create_tasks_soap(period_segment, start_date_str, page, per_page))
